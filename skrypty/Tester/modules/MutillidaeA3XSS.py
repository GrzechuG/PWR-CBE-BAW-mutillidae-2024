...
def xss_persistent(
    ...
    try:
        browser.get(
            f"http://{ip}/mutillidae/index.php?page=add-to-your-blog.php"
        )
        sleep(3)
...
def xss_reflected_1(
    ...
    try:
        browser.get(
            f"http://{ip}/mutillidae/index.php?page=dns-lookup.php"
        )
...
def xss_reflected_2(
    ...
    try:
        browser.get(
            f"http://{ip}/mutillidae/index.php?page=set-background-color.php"
        )
...
def xss_via_html_attribute(ip="192.168.255.133",
    sleep_time=0,
...
def xss_against_json(   
    ...
    try:
        browser.get(
            f"http://{ip}/mutillidae/index.php?page=pen-test-tool-lookup.php"
        )
        
        select_element = WebDriverWait(browser, 5).until(EC.presence_of_element_located((By.ID, 'idToolSelect')))
        
        # Find the option with value="1"
        option_element = select_element.find_element(By.CSS_SELECTOR, 'option[value="1"]')

        # Modify the option value using JavaScript
       
        browser.execute_script("arguments[0].value = arguments[1];", option_element, payload)
